...
- `backend/` — NestJS 10 + TS + SQLite (`better-sqlite3` + TypeORM, file `backend/data/finfin.sqlite`, `synchronize: true` creates tables on boot). Entry `src/main.ts` (port **3001**, global prefix `api`, CORS on). Entities in `src/*.entity.ts`; `AuthController` exposes `POST /api/auth/cadastro`, `POST /api/auth/login`, `POST /api/auth/logout`, `GET /api/auth/eu` (public; token = sessão aleatória via `Authorization: Bearer`, expira em 7 dias, sem deps externas — hash `scrypt` do node) plus guarded `PUT /api/auth/perfil` (nome/email/avatar dataURL ≤~375KB, `409` on taken email) and `PUT /api/auth/senha` (checks current, min 6). `Usuario.avatar` is a nullable dataURL; frontend `Avatar` in `src/ui.tsx` falls back to name initials with deterministic color. `AuthGuard` protects `AppController` (`POST/GET/PUT/DELETE /api/receitas[/:id]`, `POST/GET/PUT/DELETE /api/despesas[/:id]`, `GET /api/resumo?mes=YYYY-MM[&contaId=]`, `GET /api/contagem`, `GET /api/exportar`, `GET /api/exportar/csv?tipo=receitas|despesas` → `{filename, csv}`, `POST /api/importar` (`{modo: mesclar|substituir, backup}`, transacional), `POST /api/importar/ofx` (lote OFX já parseado, ignora FITID repetido, transacional), `DELETE /api/dados/lancamentos`, `DELETE /api/dados/tudo`) and `CatalogoController` (CRUD `/api/categorias[/:id]` (`?tipo=` filter), `/api/formas-pagamento[/:id]`, `/api/contas[/:id]`, `POST /api/restaurar` (repõe seeds que faltam)). All rows carry `usuarioId` and every query is scoped to the logged user; first signup adopts orphan rows, later signups get a seeded catalog; `409` when deleting anything still referenced by lançamentos. `Receita.fitid`/`Despesa.fitid` (nullable, FITID do OFX) é chave anti-duplicada da importação. Validation is manual in services (`BadRequestException`); `class-validator` is not installed.
- `frontend/` — React 19 + Vite 6 + TS + Tailwind CSS v4 (via `@tailwindcss/vite` plugin; styles in `src/index.css` with `@import "tailwindcss"` + `@theme`). `HashRouter` in `src/main.tsx` (`#/` routes work with any static server, no rewrite config); `/login` is public (Entrar/Criar-conta tabs), other routes render inside `RotaProtegida` (redirects to `/login` when `GET /api/auth/eu` has no user); auth state in `src/auth.tsx` (`AuthProvider` + `useAuth`), token in `localStorage` attached by `src/api.ts`; sidebar layout in `src/Layout.tsx` (shows user name + Sair); pages in `src/pages/` (Home, Lancamentos + ImportarOfx em `/lancamentos/ofx`: parse OFX em `src/ofx.ts`, mapeia conta/categorias/forma, prévia editável por linha, POST `/api/importar/ofx`, Relatorios, Categorias, FormasPagamento, Configuracoes — backup JSON/CSV, importar backup (mesclar/substituir), restaurar catálogo, apagar lançamentos/tudo com confirmação `APAGAR`); sidebar groups in `src/Layout.tsx` (Principal, Cadastros, Sistema + Perfil no rodapé); shared API client/types in `src/api.ts`, shared UI in `src/ui.tsx` (`TituloPagina` = menu icon + title), appearance prefs in `src/tema.tsx` (`ProvedorAparencia`: tema claro/escuro/sistema + largura fluida/fixa, `localStorage` `finfin_tema`/`finfin_largura`, `.dark` on `<html>` + `data-largura`; dark mode = Tailwind v4 `@custom-variant` + `dark:` variants on all surfaces); create/edit form in `src/LancamentoForm.tsx` (BRL mask, semantic accent per tipo, options from catalog API). API calls use relative `/api/...`, proxied to the backend by `vite.config.ts`.
