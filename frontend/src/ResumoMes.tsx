...
export default function ResumoMes({ resumo, carregando, mes, qtdReceitas, qtdDespesas }: Props) {
  ...
  return (
      <section aria-label={t('resumo.saldoMes')} className="grid gap-4 sm:grid-cols-3">
        <div className="rounded-2xl bg-white dark:bg-slate-900 p-5 shadow-sm ring-1 ring-slate-200 dark:ring-slate-800">
          <div className="flex items-center justify-between">
            <p className="text-sm font-medium text-slate-500 dark:text-slate-400 dark:text-slate-500">{t('resumo.receitas')} · {mesLabel(mes)}</p>
        ...
        <div className="rounded-2xl bg-white dark:bg-slate-900 p-5 shadow-sm ring-1 ring-slate-200 dark:ring-slate-800">
          <div className="flex items-center justify-between">
            <p className="text-sm font-medium text-slate-500 dark:text-slate-400 dark:text-slate-500">{t('resumo.despesas')} · {mesLabel(mes)}</p>
        ...
        <div className="rounded-2xl bg-white dark:bg-slate-900 p-5 shadow-sm ring-1 ring-slate-200 dark:ring-slate-800">
          <div className="flex items-center justify-between">
            <p className="text-sm font-medium text-slate-500 dark:text-slate-400 dark:text-slate-500">{t('resumo.saldoMes')}</p>
            <span className={`flex h-8 w-8 items-center justify-center rounded-full ${saldoPositivo ? 'bg-emerald-100 dark:bg-emerald-950' : 'bg-rose-100 dark:bg-rose-950'}`}>
              <IconeSeta direcao={saldoPositivo ? 'cima' : 'baixo'} className={`h-4 w-4 ${saldoPositivo ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`} />
            </span>
          </div>
          <p className={`mt-1 text-2xl font-bold tabular-nums ${saldoPositivo ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}>
            {carregando ? '…' : BRL.format(resumo?.saldo ?? 0)}
          </p>
          <p className="mt-1 text-xs text-slate-400 dark:text-slate-500">
            {saldoPositivo ? t('resumo.noAzul') : t('resumo.atencao')}
          </p>
        </div>
      </section>
  );
