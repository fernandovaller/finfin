...
export default function Lancamentos() {
  ...
  return (
    <main className="w-full space-y-6 px-4 py-6 sm:px-6 lg:px-8">
      ...
      <div className="space-y-6">
        ...
        >
          <h2 className="text-base font-bold">
            Receitas <span className="ml-1 text-sm font-medium text-slate-400 dark:text-slate-500">· {mesLabel(mes)} · {receitasDoMes.length}</span>
          </h2>
          {carregando ? (
        ...
        >
          <h2 className="text-base font-bold">
            Despesas <span className="ml-1 text-sm font-medium text-slate-400 dark:text-slate-500">· {mesLabel(mes)} · {despesasDoMes.length}</span>
          </h2>
          {carregando ? (
            <p className="mt-3 text-sm text-slate-400 dark:text-slate-500">Carregando…</p>
      ...
      {modalNovo && (
        <Modal titulo="Novo lançamento" onFechar={() => setModalNovo(false)} wide>
          <LancamentoForm
            key={formKey}
            tipo={tipo}
